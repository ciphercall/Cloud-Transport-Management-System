    ...
    public class GlAccountHeadController : AppController
    {
        ...
        public ActionResult IndexPost(AccountHeadModel aAccountHeadModel, string command)
        {
            ...
            if (command == "Update")
            {
                ...
                //  where (n.PARTYID == aAccountHeadModel.AcchartModel.ACCOUNTCD && n.COMPID == aAccountHeadModel.GLACCHARMSTModel.COMPID)
                //  select n;

                //foreach (CNF_PARTY x in PartyUpdate)
                //{
                //    x.PARTYNM = aAccountHeadModel.AcchartModel.ACCOUNTNM;
                //    x.UPDIPNO = ipAddress.ToString();
                //    x.UPDTIME = td;
                //    x.UPDUSERID = Convert.ToInt64(System.Web.HttpContext.Current.Session["loggedUserID"]);
                //    x.UPDLTUDE = aAccountHeadModel.GLACCHARMSTModel.INSLTUDE;
                //}
